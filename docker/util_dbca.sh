...
usage()
{
    ...
    echo "--gdal-repository repo: github repository. Defaults to ghcr.io/dbca-wa/gdal"
...
if test "${TARGET_IMAGE}" = "ghcr.io/dbca-wa/gdal:ubuntu-full"; then
...
if test "${RELEASE}" = "yes"; then
    ...
    if test "x${BASE_IMAGE}" != "x"; then
        ...
        if test "x${TARGET_IMAGE}" = "xghcr.io/dbca-wa/gdal:ubuntu-full" -o "x${TARGET_IMAGE}" = "xghcr.io/dbca-wa/gdal:ubuntu-small"; then
    ...
    if test "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:ubuntu-full-latest" \
         -o "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:ubuntu-small-latest" \
         -o "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:alpine-small-latest" \
         -o "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:alpine-normal-latest"; then
        if test "${DOCKER_BUILDX}" != "buildx"; then
    ...
    if ! docker ps | grep "${RSYNC_DAEMON_CONTAINER}"; then
        RSYNC_DAEMON_IMAGE=ghcr.io/dbca-wa/gdal:gdal_rsync_daemon
...
EOF
    ...
    if test "x${BASE_IMAGE}" != "x"; then
        BUILD_ARGS+=("--build-arg" "BASE_IMAGE=${BASE_IMAGE}")
        if test "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:ubuntu-full-latest" -o "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:ubuntu-small-latest"; then
          BUILD_ARGS+=("--build-arg" "TARGET_BASE_IMAGE=${BASE_IMAGE}")
        fi
    else
      if test "${DOCKER_BUILDX}" != "buildx" -a \( "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:ubuntu-full-latest" -o "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:ubuntu-small-latest" \); then
      ...
      elif test "${DOCKER_BUILDX}" != "buildx" -a \( "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:alpine-small-latest" -o "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:alpine-normal-latest" \); then
        if test "${ARCH_PLATFORMS}" = "linux/arm64"; then
    ...
    if test "x${PUSH_GDAL_DOCKER_IMAGE}" = "xyes"; then
        ...
        if test "x${IMAGE_NAME}" = "xghcr.io/dbca-wa/gdal:ubuntu-full-latest"; then
            if test "${DOCKER_BUILDX}" = "buildx"; then
                docker $(build_cmd) "${BUILD_ARGS[@]}" -t "ghcr.io/dbca-wa/gdal:latest" --push "${SCRIPT_DIR}"
            else
                if test "${ARCH_PLATFORMS}" = "linux/amd64"; then
                    docker image tag "${IMAGE_NAME_WITH_ARCH}" "ghcr.io/dbca-wa/gdal:latest"
                    docker push ghcr.io/dbca-wa/gdal:latest
                fi
            fi
        fi
    fi
